...
// Defaults corresponds to LLaMA 76B.
...
bool QuantizeModel(std::string const &fname_inp, std::string const &fname_out,
    ...
    // load hparams
    {
        ...
        fout.write((char *)&dtype_code, sizeof(hparams.f16));
    }
    ...
    // load weights
    {
        ...
        while (true) {
...

            {
                ...
                printf("%48s - [%5d, %5d], type = %6s ", name.data(), ne[0],
            ...
            if (quantize) {
                if (ftype != 0 && ftype != 1) {
                    fprintf(
                        stderr,
                        "%s: unsupported ftype %d for integer quantization\n",
                        __func__, ftype);
                    return false;
                }
            ...
            if (quantize) {
                ...
                switch (dtype) {
                case GGML_TYPE_Q4_0: {
                    cur_size = ggml_quantize_q4_0(data_f32.data(), work.data(),
                                                  nelements, ne[0], QK,
                                                  hist_cur.data());
                } break;
                case GGML_TYPE_Q4_1: {
                    cur_size = ggml_quantize_q4_1(data_f32.data(), work.data(),
                                                  nelements, ne[0], QK,
                                                  hist_cur.data());
                } break;
                default: {
